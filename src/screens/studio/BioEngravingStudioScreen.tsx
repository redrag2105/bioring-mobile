...
function EngravingFooter({ onReview, disabled }: { onReview: () => void; disabled: boolean }) {
    ...
    >
      <View className='flex-row items-center justify-between gap-4'>
        <View className='flex-1'>
          ...
          <Text className='font-serif text-[19px] leading-6 text-ring-primary'>{formatPrice(estimatedPrice)}</Text>
        </View>
        ...
        >
          <Text className='font-sans-bold text-[9px] uppercase tracking-[0.2em] text-white'>
            {disabled ? 'Choose Engraving' : 'Review Design'}
          </Text>
...
export function BioEngravingStudioScreen() {
  ...
  return (
    <View className='flex-1 bg-ring-background'>
            ...
            <View className='border-b border-ring-primary/10 px-5 pb-3 pt-4'>
              <View className='flex-row items-start justify-between gap-5'>
                <View className='min-w-0 flex-1'>
                  ...
                  <Text className='mt-1.5 max-w-[280px] font-sans text-[11px] leading-4 text-txt-muted'>
                    Edit the mark below while the ring preview stays pinned above.
                  </Text>
                </View>
      ...
      <EngravingFooter onReview={handleReview} disabled={!canReview} />
    </View>
  )
}
